...
func Listen(network, laddr string, config *Config) (net.Listener, error) {
	if config == nil || (len(config.Certificates) == 0 && config.GetCertificate == nil && config.GetConfigForClient == nil) {
		return nil, errors.New("tls: neither Certificates nor GetCertificate nor GetConfigForClient set in Config")
	}
...
func LoadX509KeyPair(certFile, keyFile string) (Certificate, error) {
	return LoadX509KeyPairWithPassword(certFile, keyFile, nil)
}

func LoadX509KeyPairWithPassword(certFile, keyFile string, pwd []byte) (Certificate, error) {
	...
	return X509KeyPairWithPassword(certPEMBlock, keyPEMBlock, pwd)
}
...
func X509KeyPair(certPEMBlock, keyPEMBlock []byte) (Certificate, error) {
	return X509KeyPairWithPassword(certPEMBlock, keyPEMBlock, nil)
}
func X509KeyPairWithPassword(certPEMBlock, keyPEMBlock, pwd []byte) (Certificate, error) {
	...
	if pwd != nil {
		derBytes, err = X.DecryptPEMBlock(keyDERBlock.Bytes, pwd)
		if err != nil {
			return fail(err)
		}
	} else {
	...
	cert.PrivateKey, err = parsePrivateKey(derBytes)
	if err != nil {
		return fail(err)
	}
