...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		encCertIndex := 1
		if len(certMsg.certificates) > 2 {
			encCertIndex = len(certMsg.certificates) / 2
		}
		interCerts := make([]*x509.Certificate, 0)
		...
		for i, asn1Data := range certMsg.certificates {
			...
			pubKey, _ := convertToPublicKey(cert.PublicKey)
		...
		if !c.config.InsecureSkipVerify {
			...
			for _, cert := range certs {
				c.verifiedChains, err = cert.Verify(opts)
				if err != nil {
					c.sendAlert(alertBadCertificate)
					return err
				}
			...
			for i, cert := range certs {
				err = cert.VerifyHostname(c.config.ServerName)
				if err != nil {
					c.sendAlert(alertBadCertificate)
					return err
				}
