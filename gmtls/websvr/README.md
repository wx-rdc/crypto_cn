...
使用 `easyrsa` 工具生成证书和密钥对。

相关参数保存在 `certs/vars-*` 文件中。

生成SM2双证书过程：

1. 将 `vars-sm2` 文件更名为 `vars`， 修改其中的参数。
2. 修改并执行 `./build-certs.sh`。
3. 生成的证书和密钥对保存在 `pki` 目录下。

生成RSA证书的过程类似，只需将 `vars-rsa` 文件更名为 `vars` 即可。
