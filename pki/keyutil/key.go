...
func Equal(x, y any) bool {
	...
	case ed25519.PrivateKey:
		...
		return ok && xx.Equal(yy)
	case sm2.PublicKey:
		yy, ok := y.(sm2.PublicKey)
		return ok && xx.Equal(yy)
	case sm2.PrivateKey:
		yy, ok := y.(sm2.PrivateKey)
		return ok && xx.Equal(yy)
...
func generateSM2Key() (crypto.Signer, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "error generating SM2 key")
	}
...
func generateOKPKey(crv string) (crypto.Signer, error) {
	...
	default:
		return nil, errors.Errorf("missing or invalid value for argument 'crv'. "+
			"expected 'Ed25519' or 'SM2', but got '%s'", crv)
	}
}
